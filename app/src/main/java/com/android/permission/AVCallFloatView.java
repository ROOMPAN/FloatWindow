...
import com.android.view.FloatMessageMainDialog;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (Math.abs(xDownInScreen - xInScreen) <= ViewConfiguration.get(getContext()).getScaledTouchSlop()
                    ...
                    yInScreen = event.getY() + 100;
                    // 手指移动的时候更新小悬浮窗的位置
                    updateViewPosition();
                    View view = LayoutInflater.from(context).inflate(R.layout.message_win_layout, null);
                    FloatMessageMainDialog.getInstance(context, R.style.webviewTheme).setContextView(view);
//                    Toast.makeText(getContext(), "this float window is clicked", Toast.LENGTH_SHORT).show();
    ...
    private void anchorToSide() {
        ...
        int middleX = mParams.x + getWidth() / 1;
